...
export default function Lightbox({ work, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    // Hide the site header (which has z-50 and would overlap the modal)
    const header = document.querySelector<HTMLElement>('header');
    const prevHeaderVisibility = header?.style.visibility || '';
    ...
    return () => {
      ...
      if (header) header.style.visibility = prevHeaderVisibility;
    };
